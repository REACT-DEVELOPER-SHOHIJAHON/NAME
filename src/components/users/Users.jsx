...
const Users = () => {
    ...
    if (usersIsLoading) {
        return <p className="flex items-center justify-center h-screen text-red-500">Loading...</p>;
    }
    ...
    return (
        <div className="container max-w-[1440px] mx-auto w-full flex items-center justify-center py-10">
            <div className="grid grid-cols-3 max-w-[980px] gap-6 w-full px-4">
                {users ? (
                    users.map((user) => (
                        <div to={`/details/${user.id}`} key={user.id} className="flex flex-col items-center pb-6 bg-green-600 rounded-lg shadow-md hover:shadow-lg transition-shadow" >
                            <Link to={`/details/${user.id}`} className="flex flex-col items-center w-full px-6 pt-6 pb-4">
                                <img src={user.avatar} alt={`${user.first_name} ${user.last_name}`} className="w-60 h-60 mb-4 object-cover" />
